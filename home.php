...
<?php get_header(); ?>
    ...
    <?php if (have_posts()) :
    	  while (have_posts()) : the_post(); ?>
    				...
    				<?php $the_article = ob_get_clean(); $posts_gf = array_merge($posts_gf, array($post->post_date => $the_article )); ?>
    		<?php endwhile; ?>
    		<?php endif; ?>
    		<?php query_posts('posts_per_page=-1&post_type=postitexhib'); ?>
    			  <?php if (have_posts()) : ?>
    			    <?php while (have_posts()) : the_post(); ?>
    			    <?php $terms = get_the_terms( $page->ID, 'taxonomy_name' ); ob_start(); ?>
    			    	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    			    	<div class="postpost">
    			    	<h1 class="titrepost"><?php the_title(); ?></h1>
    			    	<p class="postit"><?php the_excerpt(); ?>
